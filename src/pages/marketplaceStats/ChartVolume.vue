<template>
  <section id="barchart" class="charts card" style="--p:2em">
    <div class="toolbar wrap not_margin">
      <div class="divcol">
        ...
        <div class="values acenter gap1">
          <h6 class="p bold">{{dataVolume.value}} N</h6>
          <span class="percent" :style="`--c:${dataVolume.percent.includes('+')?'var(--success)':'var(--error)'}`">{{dataVolume.percent}}%</span>
        </div>
      </div>
    ...
    <apexchart
      ...
      :series="chartSeries"
      style="--tag: 'Volume (N)';--color: hsl(32, 93%, 54%, .8)"
    ></apexchart>
  </section>
</template>
